export default function aboutMeSkillz (p) {
  ...
  p.draw = () => {
    ...
    criticalReflectionDistance < 20 ? (isOverCritcalReflection = true, p.cursor(p.HAND)) : (isOverCritcalReflection = false, p.cursor(p.ARROW))
    ...
    collaborationDistance < 20 ? (isOverCollaboration = true, p.cursor(p.HAND)) : (isOverCollaboration = false, p.cursor(p.ARROW))
